...
! ------------------------------
     ...
     &  (abs(indata(1)).le.3) ) then
          ...
          KinE=10.**(fldata(iener))   ! Kinetic energy (GeV)
          r=10.**(fldata(idist))      ! Distance to core on ground (m) 
          phi=fldata(iphi)           ! Polar angle on ground (rad)
          wei=fldata(iweig)           ! Particle weight

          ux=fldata(iux)            ! p_x/|p| at arrival at ground
          uy=fldata(iuy)            ! p_y/|p| at arrival at ground
          umod2=ux*ux+uy*uy     
...
!          write(*,*) 'zenp (deg) = ',zenp*180./pi
          ...
          write(99,*) code, r, phi, ux, uy  ! Write azimuth angle in file
